...
#### Lessons from Makoto module (2026-05-28)

- **Every `tenant_id` column needs a FK, including child tables**: `makoto_comments.tenant_id` and `makoto_reactions.tenant_id` were written as `UUID NOT NULL` without `REFERENCES tenants(tenant_id)`. Every `tenant_id` column on every table — not just the root table — must carry the FK. When writing migrations, verify the FK is present on all three columns before applying.
- **`DELETE` must scope by `tenant_id`, not just by PK**: `deleteComment` selected by `id AND tenant_id` but then deleted by `id` alone. Any `DELETE FROM` in a multi-tenant service must include `AND tenant_id = $N` in the WHERE clause, and the corresponding test must assert `tenant_id` appears in the DELETE call parameters.
- **NestJS POST methods default to HTTP 201, not 200**: Toggle/upsert endpoints feel semantically like 200 but NestJS returns 201 for all `@Post()` handlers unless `@HttpCode(200)` is explicitly added. Write smoke script expected values to match what the controller actually returns, and add `@HttpCode` explicitly when the intended code differs from the NestJS default.
- **Validate the parent entity's tenant before inserting a child row**: `addComment` and `toggleReaction` skipped `getPost(postId, tenantId)`, so a cross-tenant or non-existent `postId` produced an unhandled 500 from a PostgreSQL FK violation. Always call the parent's `get` method at the top of any child-insert service method — it returns a clean 404 and blocks cross-tenant writes.
- **`makeUseFetch` must guard against null/empty paths**: Passing `''` to `useMakotoFetch` when a feed section is filtered out caused a real fetch to the bare API base URL, returning 404 and setting error state. The `makeUseFetch` factory must accept `string | null` and short-circuit (set `loading: false, data: null`) when path is falsy. Pass `null` — not `''` — at the call site when a fetch should be skipped.
